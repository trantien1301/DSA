...
void removeBefore(List*& L, int val)
{
    ...
    NODE* dummy = new NODE();
    dummy->p_next = L->p_head;
    NODE* curr = dummy;

    while (curr->p_next->p_next != NULL && curr->p_next->p_next->key != val) {
        curr = curr->p_next;
    }
    if (curr->p_next->p_next == NULL)
    {
        delete dummy;
        return;
    } 
...
void removeAfter(List*& L, int val)
{
    ...
    NODE* curr = L->p_head;
    while (curr->key != val && curr->p_next != NULL)
    {
        curr = curr->p_next;
    }
    if (curr->p_next == NULL||curr==NULL)
    {
        return;
    }
...
bool addBefore(List*& L, int data, int val)
{
    ...
    dummy->p_next = L->p_head;
    NODE* curr = dummy;
    while (curr->p_next != NULL&&curr->p_next->key!=val)
    {
        curr = curr->p_next;
    }
    if (curr->p_next == NULL)
