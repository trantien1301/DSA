...
void removeBefore(List *&L, int val)
{
    ...
    if (curr!=NULL && curr==L->p_head)
    {
        return;
    }
    else 
    {
    NODE *temp=curr->p_prev;
    if(temp==L->p_head)
    {
        L->p_head=curr;
        removeHead(L);
...
void removeAfter(List *&L, int val)
{
    ...
    NODE* curr = L->p_head;
    while (curr->p_next != NULL&&curr->key != val )
    {
        curr = curr->p_next;
    }
    if (curr->p_next == NULL)
    ...
    if (curr->p_next == L->p_tail)
    {
        removeTail(L);
        // NODE* needRemove = L->p_tail;
        // curr->p_next = NULL;
        // L->p_tail = curr;
        // delete needRemove;
        return;
    ...
    {
        NODE* needRemove =curr->p_next;
        needRemove->p_next->p_prev=curr;
        curr->p_next=needRemove->p_next;
        delete needRemove;
    }
    return;
}
...
bool addAfter(List *&L, int data, int val)
{
    ...
    if (curr->p_next == NULL && curr->key!=val)
    {
        return false;
    }
    NODE* newNode= createNode(data);
...
{
    // Your code here //
List* reversedlist=createList(NULL);
if (L->p_head == NULL)
{
    return reversedlist;
}
...
void removeDuplicate(List *&L)
{
    ...
    {
        NODE* check = curr;
        while (check->p_next != NULL) 
        {
            if (check->p_next->key == curr->key) 
            {
                NODE* temp = check->p_next;
                check->p_next = check->p_next->p_next;
                
                if(temp->p_next!=NULL)
                {
                    temp->p_next->p_prev=check;
                }
                if (temp == L->p_tail) {
                    L->p_tail = check;
                }
...
// --- Function main toprev->p_next = prev->p_next->p_next; test the above functions ---
